...
const DEFAULT_REPOSITORY_SLUG = "justyn-clark/space-agent";
...
function buildDownloadsTable(currentTag) {
  ...
  const rows = [
    {
      architecture: "x86",
      macos: { text: "Mac Intel", fileName: `JCN-Space-Agent-${releaseVersion}-macos-x64.dmg` },
      linux: { text: "Linux x86", fileName: `JCN-Space-Agent-${releaseVersion}-linux-x64.AppImage` },
      windows: { text: "Win x86", fileName: `JCN-Space-Agent-${releaseVersion}-windows-x64.exe` }
    },
    {
      architecture: "ARM",
      macos: { text: "Mac ARM", fileName: `JCN-Space-Agent-${releaseVersion}-macos-arm64.dmg` },
      linux: { text: "Linux ARM", fileName: `JCN-Space-Agent-${releaseVersion}-linux-arm64.AppImage` },
      windows: { text: "Win ARM", fileName: `JCN-Space-Agent-${releaseVersion}-windows-arm64.exe` }
    }
  ];
...
async function generateReleaseBodyWithOpenRouter(commits, options = {}) {
  ...
  const response = await fetch(OPENROUTER_CHAT_COMPLETIONS_URL, {
    ...
    headers: {
      ...
      "X-OpenRouter-Title": "JCN Space Agent Desktop Release Notes"
    },
