...
- `GIT_URL`: optional Git repository URL used by `node space update` and `node space supervise`; if unset they fall back to the local `origin` remote URL and only then to the canonical JCN repo URL `https://github.com/justyn-clark/space-agent.git`
